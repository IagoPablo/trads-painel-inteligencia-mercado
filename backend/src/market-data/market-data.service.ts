...
export class MarketDataService {
  ...
  async syncPopulation(referencePeriod: number) {
    ...
    const municipalityCodes = series.map((item) => item.localidade.id);
  ...
  async syncHouseholdIncome(referencePeriod: number) {
    const result =
      await this.ibgeService.getHouseholdIncomeByMunicipality(referencePeriod);
  ...
  async syncAgeGroups(referencePeriod: number) {
    ...
    for (const location of locations) {
      ...
      const municipalities = municipalitiesByState.get(location.parentId) ?? [];
    ...
    for (const municipalityCodes of municipalitiesByState.values()) {
      const batches = this.splitIntoBatches(municipalityCodes, batchSize);

      for (const batch of batches) {
        const result = await this.ibgeService.getAgeGroupsByMunicipalities(
          batch,
          referencePeriod,
        );
        ...
        for (const resultItem of results) {
          ...
          const ageCategoryCodes = Object.keys(ageClassification.categoria);
  ...
  private buildLocationWhere(filters: MarketDataFiltersDto) {
    ...
    const stateCode = state ? STATE_CODES[state] : undefined;
  ...
  async findMarketData(filters: FindMarketDataDto) {
    ...
    const sortIndicator =
      filters.sortBy === 'householdIncome' ? 'HOUSEHOLD_INCOME' : 'POPULATION';
    ...
    if (filters.municipality) {
      const selectedIndicator = await this.prisma.marketIndicator.findFirst({
        where: {
          indicator: sortIndicator,
          referencePeriod: 2022,
          location: locationWhere,
      ...
      if (selectedIndicator) {
        const rankingLocationWhere = this.buildLocationWhere({
          ...filters,
          municipality: undefined,
        });

        const betterIndicators = await this.prisma.marketIndicator.count({
          where: {
            indicator: sortIndicator,
            referencePeriod: 2022,
  ...
  async getMunicipalityMarketData(ibgeCode: string) {
    ...
    const population = indicators.find(
      (indicator) => indicator.indicator === 'POPULATION',
    );

    const householdIncome = indicators.find(
      (indicator) => indicator.indicator === 'HOUSEHOLD_INCOME',
    );

    const ageGroups = Object.fromEntries(
      indicators
        .filter((indicator) => indicator.indicator === 'AGE_GROUP')
        .map((indicator) => [indicator.dimension, Number(indicator.value)]),
    );
    ...
    return {
      ...
      referencePeriod: 2022,
      population: population ? Number(population.value) : null,
      householdIncome: householdIncome ? Number(householdIncome.value) : null,
      ageGroups,
    };
  }
}
