...
import { MarketDataSortBy, SortOrder } from './dto/find-market-data.dto';
...
describe('MarketDataService', () => {
  ...
  beforeEach(() => {
    ...
    service = new MarketDataService(prisma as never, ibgeService as never);
  });
  ...
  it('should reject an invalid state', async () => {
    ...
    expect(prisma.marketIndicator.findMany).not.toHaveBeenCalled();

    expect(prisma.marketIndicator.count).not.toHaveBeenCalled();
  });
  ...
  it('should return formatted market data', async () => {
    ...
    expect(prisma.marketIndicator.findMany).toHaveBeenCalledTimes(2);

    expect(prisma.marketIndicator.findFirst).toHaveBeenCalledTimes(1);

    expect(prisma.marketIndicator.count).toHaveBeenCalledTimes(2);
  });
  ...
  it('should calculate the market data summary', async () => {
    ...
    expect(prisma.marketIndicator.findMany).toHaveBeenCalledTimes(1);
  });
