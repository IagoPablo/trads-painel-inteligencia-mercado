...
function MarketSummary({
  ...
  const ageGroupPopulation = ageGroup ? summary.ageGroups[ageGroup] : 0;

  const isMunicipalitySelected = Boolean(municipality);

  const locationName = municipality ? municipality : state ? state : "Brasil";

  return (
    <section className="market-summary">
      <article className="market-summary-card">
        <span>{isMunicipalitySelected ? "Mercado" : "Municípios"}</span>
      ...
      <article className="market-summary-card">
        ...
        <strong>{summary.population.toLocaleString("pt-BR")}</strong>
      </article>
      ...
      <article className="market-summary-card">
        <span>{ageGroup ? `Público ${ageGroup} anos` : "Público"}</span>
